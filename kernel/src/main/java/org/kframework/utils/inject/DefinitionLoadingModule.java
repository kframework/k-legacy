...
import org.kframework.definition.ProcessedDefintion;
...
public class DefinitionLoadingModule {
    ...
    public static ProcessedDefintion miniKoreDefinition(BinaryLoader loader, FileUtil files) {
        MiniKore.Definition koreDefinition = new TextToMini().parse(files.resolveKompiled(FileUtil.KORE_TXT));
        KompileOptions kompileOptions = loader.loadOrDie(KompileOptions.class, files.resolveKompiled(FileUtil.KOMPILE_OPTIONS_BIN));
        return new ProcessedDefintion(kompileOptions, koreDefinition);
    }
