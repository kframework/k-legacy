...
trait Rewriter {
  ...
  def `match`(k: kore.K, rule: Rule): java.util.List[Tuple2[_ <: java.util.Map[_ <: kore.KVariable, _ <: kore.K], _ <: kore.K]]
   ...
   */
  def search(initialConfiguration: kore.K, depth: Optional[Integer], bound: Optional[Integer], pattern: Rule, searchType: SearchType): java.util.List[Tuple2[_ <: java.util.Map[_ <: kore.KVariable, _ <: kore.K], _ <: kore.K]]

  def executeAndMatch(k: kore.K, depth: Optional[Integer], rule: Rule): Tuple2[RewriterResult, java.util.List[Tuple2[_ <: java.util.Map[_ <: kore.KVariable, _ <: kore.K], _ <: kore.K]]]
