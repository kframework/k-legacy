...
trait Constructors[KK <: K] {
  ...
  def KToken(s: String, sort: Sort, att: Att): KK with KToken
  def KApply(klabel: KLabel, klist: KList, att: Att): KK with KApply
  def KSequence[KKK <: KK](items: java.util.List[KKK], att: Att): KK with KSequence
  def KVariable(name: String, att: Att): KVariable with KK with KVariable
  def KRewrite(left: K, right: K, att: Att): KRewrite with KK with KRewrite
  ...
  @annotation.varargs def KApply(klabel: KLabel, items: KK*): KApply with KK = KApply(klabel, KList(items.asJava), emptyAtt)
