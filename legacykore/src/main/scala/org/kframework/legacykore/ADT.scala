...
object ADT {
  ...
  case class KLabelLookup(name: String) extends legacykore.KLabel {
    override def toString = name

    def apply(ks: K*) = KApply(this, KList(ks.toList))
  }

  case class KLabel(module: Module, localName: String) extends legacykore.KLabel {
  ...
  case class KApply[KK <: K](klabel: legacykore.KLabel, klist: legacykore.KList, att: Att = Att()) extends legacykore.KApply {
    def items = klist.items
    def size = klist.size
    def asIterable = klist.asIterable
  }

  class KSequence private(val elements: List[K], val att: Att = Att()) extends legacykore.KSequence {
    val items: java.util.List[K] = elements.asJava
    val size: Int = elements.size
    val asIterable: java.lang.Iterable[K] = new org.kframework.List(elements)
    lazy val kApply: legacykore.KApply = items.asScala reduceRightOption { (a, b) => KLabelLookup(KLabels.KSEQ)(a, b) } getOrElse {KLabelLookup(KLabels.DOTK)()} match {
      case k: legacykore.KApply => k
  ...
  case class KVariable(name: String, att: Att = Att()) extends legacykore.KVariable {
    def apply(ks: K*) = KApply(this, KList(ks.toList))
  }
  ...
  case class SortLookup(localName: String, moduleName: ModuleName) extends legacykore.Sort with LookupSymbol {
    override def toString = name

    override def equals(other: Any) = other match {
      case s: Sort if this.moduleName == ModuleName.STAR => s.localName == this.localName
      case s: legacykore.Sort => this.moduleName == s.moduleName && this.localName == s.localName
  ...
  case class Sort(localName: String, moduleName: ModuleName) extends legacykore.Sort with ResolvedSymbol {
    override def toString = name
    assert(moduleName != ModuleName.STAR)

    override def equals(other: Any) = other match {
      case s: SortLookup if s.moduleName == ModuleName.STAR => this.localName == s.localName
      case s: legacykore.Sort => s.moduleName == this.moduleName && s.localName == this.localName
  ...
  case class KToken(s: String, sort: legacykore.Sort, att: Att = Att()) extends legacykore.KToken

  case class KList(elements: List[K]) extends legacykore.KList {
  ...
  case class KRewrite(left: legacykore.K, right: legacykore.K, att: Att = Att()) extends legacykore.KRewrite

  case class InjectedKLabel(klabel: legacykore.KLabel, att: Att) extends legacykore.InjectedKLabel

}

object SortedADT {

  case class SortedKVariable(name: String, att: Att = Att()) extends legacykore.KVariable {
